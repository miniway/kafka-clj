...
(defn build-message
  ...
  (let [magic       (or (:magic opts) MAGIC-COMPRESSION)
        compression (or (:compression.codec opts) COMPRESSION-NO-COMPRESSION) ;  #FIXME: compressed.topics
        topic-bytes (.getBytes topic)]  
    (doto buffer
    ...
    (let [pos (.position buffer)
       ...
       (doseq [partial-messages messages]
            (doseq [m partial-messages]
              (let [bytes (.bytes (pack m))
                    crc   (crc32-int bytes)
                    size  (+ (alength bytes) 6)] ; message-length + 6 (crc + magic + compression)
                (doto child-buffer
                  (.putInt size)
                  (.put (byte magic))
                  (.put (byte compression))
                  (.putInt crc)
                  (.put bytes)))))
...
(defn build-request
  "Build request"
  [buffer topic partition messages & [opts]]
  (doto buffer
    (.clear)
...
(defn- broker-handler
  "Send messages."
  [[^long broker-id channel] opts evt sequence end]
  (when (= (.getKey evt) broker-id)
    (.write channel (.getValue evt))
  ))
...
(defn producer
  ...
  (let [tick  (let [t (atom -1)] #(swap! t inc))
        ...
        disruptor (new-disruptor (map #(partial broker-handler % conf) brokers) 
                                 :mt multithread 
                                 :factory-options [(or (:max.message.size conf) 100000)])
        _ (.start disruptor)
